...
export default function ProfileScreen({ navigation }: any) {
  ...
  const loadData = async () => {
    ...
    try {
      const resUsers = await fetch("https://89fe7478329a133b.mokky.dev/Users");
      ...
      Alert.alert("Ошибка", "Не удалось загрузить профиль или склады");
    }
  };
  ...
  const logout = async () => {
    await AsyncStorage.multiRemove(["authToken", "selectedStockId", "userEmail"]);
    navigation.reset({
      index: 0,
    ...
    <View style={styles.container}>
      ...
      {user ? (
        <>
          ...
          {user.phone && <Text style={styles.info}>Телефон: {user.phone}</Text>}
        </>
      ) : (
